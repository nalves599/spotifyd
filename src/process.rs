...
pub(crate) fn spawn_program_on_event(
    ...
    match event {
        PlayerEvent::PlayRequestIdChanged { play_request_id } => {
            env.insert("PLAYER_EVENT", "playrequestid".to_string());
            env.insert("PLAY_REQUEST_ID", play_request_id.to_string());
        },
        PlayerEvent::TrackChanged {
            audio_item
        } => {
            // TODO: Expose rest of audio item fields
        ...
        PlayerEvent::VolumeChanged { volume } => {
            env.insert("PLAYER_EVENT", "volumeset".to_string()); // TODO: Change string to
                                                                 // volumechange?
            env.insert("VOLUME", volume.to_string());
        }
        PlayerEvent::PositionCorrection { play_request_id, track_id, position_ms }
        => {
        ...
        PlayerEvent::Seeked { play_request_id, track_id, position_ms } => {
            env.insert("PLAYER_EVENT", "seek".to_string());
            env.insert("PLAY_REQUEST_ID", play_request_id.to_string());
            env.insert("TRACK_ID", track_id.to_base62().unwrap());
            env.insert("POSITION_MS", position_ms.to_string());
        }
        ...
        PlayerEvent::SessionConnected { connection_id, user_name } => {
            ...
            env.insert("CONNECTION_ID", connection_id.to_string());
        ...
        PlayerEvent::SessionClientChanged { client_id, client_name, client_brand_name, client_model_name } => {
            env.insert("PLAYER_EVENT", "sessionclientchanged".to_string());
            env.insert("CLIENT_ID", client_id.to_string());
